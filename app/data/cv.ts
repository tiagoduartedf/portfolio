  ...
  experience: [
    {
      ...
      summary: {
        en: "For context, same text from About above:\n\nResponsible for the DLOA AI platform, a B2B omnichannel communication solution serving the largest pharmaceutical companies in Brazil. The system integrates with almost all pharmacies in the country, along with call centers and websites: every patient adherence or medication purchase triggers the right communication sequence for that patient. I work on the project end-to-end: design, front-end, api, database, deploy and monitoring. The platform processes tens of millions of flow executions per month.\n\nThe heart of the system is the flow, modeled as two entities.\n\nConfiguration: the campaign design, assembled visually by the marketing team (low-code style). A linked list of typed blocks (send, wait, conditional, tag...) with their own properties, each pointing to the next. Waits accept a specific day, X days, or a patient variable (compliance date, birthday); conditionals branch to multiple targets based on patient data, and a flow can grow to dozens of blocks splitting in many directions.\n\nExecution: a runtime instance that runs a patient through a configuration, with history, a pointer to the current block, and its own state. Many run in parallel (one per patient), and at each block they apply the type's effect: a send dispatches the message, a conditional evaluates the rules and picks the branch, a wait holds the pointer until the time condition fires.\n\nThe whole thing lives as a state machine persisted in PostgreSQL via TypeORM.\n\nFront-end: a React SPA with TypeScript, Reactflow powering the visual flow builder, TanStack Query handling cache/revalidation/mutations for the data-heavy screens, and Chart.js for the operators' KPI dashboards.\n\nBack-end: NestJS API on Node.js. Multi-tenant SaaS with layered permissions: per industry (gold/silver/bronze plan + addons like pharmacovigilance) and per user (admin, viewer, operator, moderator). Routes documented in Swagger, JWT auth with tenant + plan claims feeding the RBAC.\n\nMessaging: producer/consumer on Azure Service Bus. Workers dispatch to the right provider within each channel's rate-limits (a WhatsApp BSP, two SMS gateways, an email ESP). The queue controls scale and respects per-domain, per-IP and per-phone rate-limits, absorbing campaigns that enqueue millions of sends at once.\n\nInfra: managed cloud behind a load balancer, observability isolated on its own host, automated CI/CD, plus auxiliary microservices (e.g. email validator) and a separate staging environment.\n\nObservability: full Grafana stack. Prometheus for metrics, Pino + Loki for structured logs, OpenTelemetry + Tempo for distributed tracing, and Alertmanager for paging.",
        pt: "Pra contexto, mesmo texto do Sobre mim acima:\n\nResponsável pela plataforma do DLOA AI, solução B2B de comunicação omnichannel que atende as maiores indústrias farmacêuticas do Brasil. O sistema integra com quase todas as farmácias do país, além de call centers e sites: cada adesão ou compra de medicamento dispara a régua de comunicação certa pro paciente. Atuo no projeto de ponta a ponta: design, front-end, api, banco de dados, deploy e monitoramento. A plataforma processa dezenas de milhões de execuções de fluxo por mês.\n\nO coração do sistema é o flow, modelado em duas entidades.\n\nConfiguração: o desenho da campanha, montado visualmente pelo time de marketing (tipo low-code). Lista encadeada de blocos tipados (envio, espera, condicional, tag...) com propriedades próprias, cada um apontando pro próximo. Esperas aceitam dia específico, X dias ou variável do paciente (compliance, aniversário); condicionais ramificam pra múltiplos destinos com base nos dados do paciente, e uma régua pode crescer pra dezenas de blocos em muitas direções.\n\nExecução: instância runtime que roda um paciente dentro de uma configuração, com histórico, ponteiro pro bloco atual e estado próprio. Várias rodam em paralelo (uma por paciente), e em cada bloco aplicam o efeito do tipo: envio dispara a mensagem, condicional avalia as regras e escolhe a ramificação, espera segura o ponteiro até a condição temporal disparar.\n\nTudo persistido como máquina de estados em PostgreSQL via TypeORM.\n\nFront-end: SPA em React com TypeScript, Reactflow sustentando o builder visual de fluxos, TanStack Query gerenciando cache/revalidação/mutações das telas pesadas em dados, e Chart.js nos dashboards de KPI dos operadores.\n\nBack-end: API NestJS sobre Node.js. SaaS multi-tenant com permissionamento em camadas: por indústria (plano gold/silver/bronze + addons como farmacovigilância) e por usuário (admin, viewer, operator, moderator). Rotas documentadas no Swagger, autenticação JWT com claims de tenant + plano alimentando o RBAC.\n\nMensageria: producer/consumer no Azure Service Bus. Workers despacham pro provedor certo dentro dos rate-limits de cada canal (um BSP de WhatsApp, dois gateways de SMS, um ESP de email). A fila controla escala e respeita rate-limits por domínio, IP e número de telefone, absorvendo campanhas que enfileiram milhões de envios de uma vez.\n\nInfra: cloud gerenciada atrás de load balancer, observabilidade isolada em host próprio, CI/CD automatizado, microserviços auxiliares (ex.: validador de email) e ambiente de homologação separado.\n\nObservabilidade: stack Grafana completo. Prometheus pras métricas, Pino + Loki pros logs estruturados, OpenTelemetry + Tempo pro tracing distribuído, e Alertmanager pra paging.",
      },
      bullets: {
